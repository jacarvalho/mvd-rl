...
class ReplayMemory(Serializable):
    ...
    def get(self, n_samples):
        ...
        if self.size < n_samples:
            n_samples = -1
        if n_samples == -1:
            s = self._states[:self.size]
            a = self._actions[:self.size]
            r = self._rewards[:self.size]
            ss = self._next_states[:self.size]
            ab = self._absorbing[:self.size]
            last = self._last[:self.size]
        else:
            s = list()
            a = list()
            r = list()
