...
class LQR(Environment):
    ...
    def generate(dimensions=None, s_dim=None, a_dim=None, max_pos=np.inf, max_action=np.inf, eps=.1,
        ...
        """
        Factory method that generates an lqr.
        ...
        A = np.random.uniform(-1., 1., (s_dim, s_dim))
        B = np.random.uniform(-1., 1., (s_dim, a_dim))

        L_Q = np.random.uniform(-1., 1., (s_dim, s_dim)) * scale
        Q = np.dot(L_Q, L_Q.T)
        L_R = np.random.uniform(-1., 1., (a_dim, a_dim)) * scale
        R = np.dot(L_R, L_R.T)
