...
class GaussianTorchPolicy(TorchPolicy):
    """
    Torch policy implementing a Gaussian policy with optional trainable standard
    ...
    def __init__(self, network, input_shape, output_shape, std_0=1., trainable_std=True,
        ...
        self._trainable_std = trainable_std
        self._log_sigma = nn.Parameter(log_sigma_init, requires_grad=trainable_std)
    ...
    def draw_action_t(self, state, reparametrize=False):
        if reparametrize:
            return self.distribution_t(state).rsample()
        else:
    ...
    def parameters(self):
        if self._trainable_std:
            ...
            return self._mu.model.network.parameters()
